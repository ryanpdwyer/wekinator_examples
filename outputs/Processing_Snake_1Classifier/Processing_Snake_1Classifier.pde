...
// Currently set up so that
// class 1 = up, class 2 = down, class 3 = left, class 4 = right.
// class 5 = restart game...
...
void setup()
{
  ...
  dest = new NetAddress("127.0.0.1",6448); //send messages back to Wekinator on port 6448, localhost (this machine) (defau
  restart();
  size(400,400);
  textAlign(CENTER);
}
...
void oscEvent(OscMessage theOscMessage) {
  if (theOscMessage.checkAddrPattern("/wek/outputs") == true) {
    ...
    // Up: Class 1
    if (f == 1 && angle!=270 && (heady[1]-8)!=heady[2])
    {
      angle=90;
    }
    // Down: Class 2
    if (f == 2 && angle!=90 && (heady[1]+8)!=heady[2])
    {
      angle=270;
    }
    // Left: Class 3
    if (f == 3 && angle!=0 && (headx[1]-8)!=headx[2])
    {
      angle=180;
    }
    // Right: Class 4
    if (f == 4 && angle!=180 && (headx[1]+8)!=headx[2])
    {
      angle=0;
    }
    if (f == 5) {
