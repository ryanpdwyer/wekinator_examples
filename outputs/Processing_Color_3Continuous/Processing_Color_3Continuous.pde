...
void oscEvent(OscMessage theOscMessage) {
 if (theOscMessage.checkAddrPattern("/wek/outputs")==true) {
     if(theOscMessage.checkTypetag("fff")) { // looking for 1 control value
        ...
        float receivedVal = theOscMessage.get(2).floatValue();
        myHue = map(receivedHue, 0, 1, 0, 255);
        mySat = map(receivedSat, 0, 1, 0, 255);
        myVal = map(receivedVal, 0, 1, 0, 255);
     } else {
...
void drawtext() {
    ...
    textAlign(LEFT, TOP); 
    fill(0, 0, 255);
    text("Receiving 3 continuous parameters:\nhue, saturation, value in range 0-1", 10, 10);
